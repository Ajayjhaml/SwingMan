...
public class Stickman : MonoBehaviour
{
    ...
    [Header("Distance Bar UI")]
    public Slider distanceBar;                   // assign in inspector (Min=0, Max=1)
    public TextMeshProUGUI distanceText;         // optional: assign TMP text for numeric distance
    public Transform finishLine;                 // assign finish line transform in inspector
    ...
    private void Start()
    {
        ...
        // Initialize distance bar
        if (finishLine != null)
        {
            startDistance = Vector2.Distance(transform.position, finishLine.position);
            // protect against zero or extremely small distances
            if (startDistance <= 0f) startDistance = 0.0001f;
        }
        ...
        if (distanceBar != null)
        {
            ...
            distanceBar.value = 1f;
        }

        if (distanceText != null && finishLine != null)
        {
            float currentDistance = Vector2.Distance(transform.position, finishLine.position);
    ...
    private void Update()
    {
        ...
        // Update distance bar every frame (safe: does nothing if references missing)
        UpdateDistanceBar();
    }

private void UpdateDistanceBar()
{
    if (finishLine == null || distanceBar == null) return;

    float currentDistance = Vector2.Distance(transform.position, finishLine.position);

    // Normalize so that: far = 0, near = 1 (fills left → right)
    float normalizedValue = (startDistance > 0f)
